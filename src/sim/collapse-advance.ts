...
export function updateCollapseAdvances(world: WorldState): void {
  ...
  for (const advance of state.advances) {
    ...
    if (controlledBy(world, advance.currentTargetRegionId) === advance.nationId) {
      ...
      const next = selectTarget(world, advance.nationId, advance.enemyNationId, sectorsFor(world, advance.nationId, advance.enemyNationId), advance.currentTargetRegionId, new Set(advance.targetRegionIds));
  ...
  for (const assessment of assessments) {
    ...
    if (units.length === 0) { state.artificialInactivityRemaining += 1; continue; }
    const advance: CollapseAdvance = { nationId: assessment.nationId, enemyNationId: assessment.enemyNationId, sourceSectorId: opportunity.sector.id, phase: "forming", unitIds: units.map((unit) => unit.id), targetRegionIds: [opportunity.target.regionId], currentTargetRegionId: opportunity.target.regionId, startedAtTick: world.time.fastTick, phaseStartedAtTick: world.time.fastTick, reasonFlags: ["no-valid-frontline-target", "enemy-defense-collapsed", "reachable-open-territory"], occupiedTargetCount: 0, stopReason: null };
...
function assessOpportunity(world: WorldState, nationId: NationId, enemyNationId: NationId) {
  if (getCapitalDefenseAssessment(world, nationId)?.threatLevel === "critical") return null;
  if (getOffensiveOperations(world, nationId).some((operation) => operation.enemyNationId === enemyNationId && operation.phase === "recovering")) return null;
  ...
  const enemyDefense = sectors.reduce((sum, sector) => sum + (getFrontlineCoverage(world, sector.id, enemyNationId)?.defenderStrength ?? 0), 0);
  if (enemyDefense > WORLD_BALANCE.war.landFront.collapseAdvance.collapsedDefenseStrength) return null;
  const candidates = sectors.map((sector) => ({ sector, target: selectTarget(world, nationId, enemyNationId, [sector]) })).filter((item): item is { sector: OperationalSector; target: NonNullable<ReturnType<typeof selectTarget>> } => !!item.target).sort((a, b) => a.target.distance - b.target.distance || compareIds(a.target.regionId, b.target.regionId));
...
function selectTarget(world: WorldState, nationId: NationId, enemyNationId: NationId, sectors: OperationalSector[], origin?: MesoRegionId, excluded = new Set<MesoRegionId>()) {
